...
fn find_key_biggest_val<K, V>(a_hash_map: &HashMap<K, V>) -> Option<&K>
where
    V: Ord,
{
    a_hash_map
...
fn find_hand_strength(hand: &Hand) -> i32 {
    ...
    let jokers = found_map.get(&'J');
    let mut jokers_count: i32 = 0;
    match jokers {
        Some(x) => {
            jokers_count = *x;
        }
    ...
    if jokers_count < 5 {
        found_map.remove(&'J');
        let key_biggest_val = find_key_biggest_val(&found_map);

        let biggest_key_val = found_map.get_mut(&key_biggest_val.unwrap().clone());
        match biggest_key_val {
            Some(x) => {
                *x += jokers_count;
            }
            None => {}
